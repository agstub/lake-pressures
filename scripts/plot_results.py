...
def plot(t_ref,timesteps=range(Nt),h_lim=5,w_lim=5):
    ...
    for i in timesteps:
        ...
        if np.size(timesteps) == 1:
            plt.axvline(x=t0[timesteps[0]],color='deeppink',ymax=0.2,linestyle='-',linewidth=3,)
            plt.annotate(xy=(t0[timesteps[0]]-0.05,-0.275 ),color='deeppink',text=r'(a)',fontsize=14)
        ...
        ax2 = plt.gca().twinx()
        dd = 0.5 # spacing for ticks
        disp = 7 # displacement so ticks appear near bottom of plot
